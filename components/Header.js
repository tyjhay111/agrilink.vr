const Header = () => {
  ...
  return (
    <header className={`sticky top-0 z-40 transition-all duration-300 ${isSolid ? 'bg-white/90 backdrop-blur-sm shadow-sm' : 'bg-transparent'}`}>
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          {/* Logo */}
          <a href="index.html" className="flex items-center space-x-2">
            <img src="images/agrlink.jpg" alt="AgriLink Logo" loading="lazy" className="w-10 h-10 rounded-full object-cover"/>
          ...
          <nav className="hidden md:flex items-center space-x-6" aria-label="Primary navigation">
            {navLinks.map((link) => {
              ...
              return (
                <a key={link.href} href={link.href} className={`pb-1 ${isActive ? 'text-[var(--primary-color)] font-semibold border-b-2 border-[var(--primary-color)]' : 'text-gray-600 hover:text-[var(--primary-color)] font-medium transition-colors'}`}>
                  {link.text}
                </a>
              );
            })}
